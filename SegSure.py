...
from PyQt5.QtWidgets import QGraphicsView, QGraphicsScene, QGraphicsPixmapItem, QFrame, QPushButton, QHBoxLayout, QVBoxLayout, QLabel, QCheckBox, QComboBox, QApplication, QFileDialog, QWidget, QMessageBox
...
class DataLoader():
    ...
    def prep_df(self):
        ...
        self.summary_df["bp_err_reason"] = self.summary_df.get("bp_err_reason", "")
    ...
    def get_random_sample(self, number: int=100):
        ...
        """
        self.random_df = self.summary_df(number)
        self.random_df.reset_index(drop=True)
...
class SelectPathsDialog(QWidget):
    def __init__(self, parent):
        ...
        self.segmentations_button.clicked.connect(self.select_segmentations_folder)
    ...
    def select_images_folder(self):
        ...
        folder = QFileDialog.getExistingDirectory(self, "Select Images Folder", options=options)
        if folder:
    ...
    def select_segmentations_folder(self):
        options = QFileDialog.Options()
        options |= QFileDialog.ReadOnly
        folder = QFileDialog.getExistingDirectory(self, "Select Segmentations Folder", options=options)
        if folder:
            # Save the selected folder to a class variable
    ...
    def confirm_and_exit(self):
        try:
            self.parent.load_data([self.images_folder, self.segmentations_folder, self.csv_file])
            self.hide()
...
class SegReviewer(QWidget):
    def __init__(self):
        ...
        # Create checkboxes
        error_checkbox = QCheckBox("Error", self)
        reviewed_checkbox = QCheckBox("Reviewed", self)
    ...
    def load_case(self, increment: bool):
        ...
        def _load_values(pid):
            ...
            tlv_col = get_color(tlv, 3.5, 8.0)
            tav_col = get_color(tav, 0.08, 0.4)
            tac_col = get_color(tac, 150, 400)
        ...
        def _get_pid(increment):
            if increment:
                ...
                self.idx -= 1
            if abs(self.idx) >= self.review_df.shape[0]:
                    self.idx = 0
